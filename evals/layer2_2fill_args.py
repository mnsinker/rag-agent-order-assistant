...
from utils.arg_utils import fill_args_from_contextd
...
cases = [
    { # Case 1：纯 args（无依赖）
        ...
        "tool_results": [RefundEligibilityDTO(True, "not shipped")],
        "expected": {"error": False,
                     "params": {"order_id": "123", "eligibility": RefundEligibilityDTO}}
    ...
    { # Case 2: context 补参数
        ...
        "resolved_args": {},
        "tool_results": [OrderSummaryDTO(order_id="456", user_id="u2", days=4, shipped=False, custom=False, amount=800)],
        "expected": {"error": False,
                     "params": {"user_id": "u2"}}
    },
    { # Case 3: 覆盖 llm args
        ...
        "tool_results": [OrderSummaryDTO(order_id='123', user_id="u1", days=3, shipped=True, custom=False, amount=1000)],
        "expected": {"error": False,
                     "params": {"user_id": "u1"}}
    },
    { # Case 4: 缺 dependency
    ...
    { # Case 5: 多个结果（取最近）
        ...
        "resolved_args": {},
        "tool_results": [
            OrderSummaryDTO(order_id='123', user_id="u1", days=3, shipped=True, custom=False, amount=1000),
            OrderSummaryDTO(order_id="456", user_id="u2", days=4, shipped=False, custom=False, amount=800)
        ],
        "expected": {"error": False,
                     "params": {"user_id": "u2"}}
    }
